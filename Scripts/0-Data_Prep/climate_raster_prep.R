...
for(i in 1:length(ncPar)){
  ...
  crs(par6190)<-crs_lcc
  crs(par8110)<-crs_lcc
  par6190<-projectRaster(par6190 ,crs=crs_wgs84)
  par8110<-projectRaster(par8110, crs=crs_wgs84)
  par6190 <- crop(par6190,extent(-124.8, -104, 32.5, 49))
  par8110 <- crop(par8110,extent(-124.8, -104, 32.5, 49))
  
  writeRaster(par6190,paste("Data/Environmental/Processed/CMIP6190",par.nc,sep="/"))
  writeRaster(par8110,paste("Data/Environmental/Processed/CMIP8110",par.nc,sep="/"))
  plot(par6190)
  parDif <- par8110 - par6190
  names(parDif) <- paste("d_",par,sep="")
  writeRaster(parDif,paste("Data/Environmental/Processed/CMIPdif/","d_",par.nc,sep=""))
